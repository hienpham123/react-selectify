...
function useSelection({ selectedKeys, options, multiple, onChange }) {
    ...
    const handleSelection = React.useCallback((event, option, checked) => {
...
function useDropdownPosition({ isOpen, positionOffset, dropdownRef, inputRef }) {
    const [actualPosition, setActualPosition] = React.useState(positionOffset || 'bottom');
...
const OptionItem = ({ option, isSelected, isHighlighted, multiple, showTooltip, renderOption, onSelect, optionRef }) => {
    ...
    const handleCheckboxChange = React.useCallback((ev, checked) => {
        if (option.disabled)
            return;
        if (ev) {
            ev.stopPropagation();
    ...
    if (renderOption) {
        return (React__default["default"].createElement("label", Object.assign({}, commonProps), renderOption({
            option,
            selected: isSelected,
    ...
    return (React__default["default"].createElement("label", Object.assign({}, commonProps),
        multiple && (React__default["default"].createElement("span", { className: "checkbox-wrapper", onClick: handleWrapperClick },
            React__default["default"].createElement(react.Checkbox, { ariaLabelledBy: option.key, ariaDescribedBy: option.text, checked: isSelected, onChange: handleCheckboxChange, disabled: option.disabled }))),
        React__default["default"].createElement("span", { className: "option-text" }, option.text)));
};
// Memoize component to prevent unnecessary re-renders
// Note: We don't compare onSelect and optionRef as they are functions and will always be different
React__default["default"].memo(OptionItem, (prevProps, nextProps) => {
...
function ReactSelectify(props) {
    ...
    const calculatedPosition = React.useMemo(() => {
        if (!isOpen) {
    ...
    const handleSelect = React.useCallback((event, option, checked) => {
    ...
    return (React__default["default"].createElement("div", { className: `hh-Multi-Select ${className}`, ref: dropdownRef, style: Object.assign({}, styles['root']) },
        React__default["default"].createElement("div", { className: `hh-Input-Wrapper ${disabled ? 'disabled' : ''}` },
            React__default["default"].createElement(react.TextField, { componentRef: inputRef, placeholder: getDisplayValue() || placeholder, onClick: toggleDropdown, onKeyDown: handleInputKeyDown, onChange: handleInputChange, value: getInputValue(), className: "hh-Input-Display", disabled: disabled, style: Object.assign({}, styles['input']), autoComplete: "off", readOnly: !isOpen && multiple }),
